...
export function ChoresList({ onEdit }: ChoresListProps) {
  ...
  const handleComplete = async (choreId: number) => {
    ...
    // Без assigned_to — просто отмечаем как выполненное
  ...
  // Сортировка дел: просроченные сверху, потом на сегодня, потом остальные
  const sortedChores = [...chores].sort((a, b) => {
    const statusA = getStatus(a.next_due);
    const statusB = getStatus(b.next_due);
    const statusOrder = { overdue: 0, today: 1, future: 2 };
  ...
  return (
    <Reorder.Group axis="y" values={sortedChores} onReorder={handleReorder}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 flex items-center gap-2">
                      ...
                      <span className={`${
                        ...
                        status === 'today' ? 'text-warning' : 'text-success'
                      }`}>
                        {styles.label}
                      </span>
                    </div>
